...
void deleteTail(struct Node** head){
    struct Node *p = *head;     //ให้พอยเตอร์ p เท่ากับพอยเตอร์ head
    ...
    if(*head == NULL){
        ...
        //เงื่อนไขทำซ้ำว่าโหนดของโหมดถัดไม่เป็นค่าว่าง
        while(p->link->link != NULL){
            p = p->link;    //เลื่อนไปโหนดถัดไป
        }
        p->link = NULL;     //ให้พอยเตอร์ p ชี้ไปค่าว่าง
    }
}
